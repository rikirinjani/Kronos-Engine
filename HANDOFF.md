...
**Branch Analyst:** Post-paper. My pipeline (diff-engine, stats, sensitivity sweep, experiment runner) is fully domain-agnostic — it operates on `SectorState` objects and never references Indonesia, GDP, or healthcare. Running a Somnium counterfactual would be identical to running P-003: `loadEraAndRun(fictionalEra, rewindPoint, {ticks, seeds})`. Zero new experiment infrastructure needed. BUT: I can't run anything without sector implementations that understand fictional worlds. The bottleneck is Sector Engineer's fictional sector logic, not my layer. Since there's nothing for me to do during paper mode, and the capability doesn't decay by waiting, I concur: ship JAMIA first, then Somnium. The one caveat: if Sector Engineer wants to build fictional sectors as a side lane (no timeline pressure), I can run a "Send the Eagles" counterfactual in about 30 minutes of experiment wiring — so the marginal cost per counterfactual once the sectors exist is essentially zero.
