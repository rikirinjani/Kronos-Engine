...
### Pending
- **Future era (S-BASELINE).** ERA-INDEX.md lists a Future era (2026+) as "(generated by engine)" but no spec or data exists. Define what a forward-projection era looks like: starting from RP-CONTEMP-003 (Baseline 2026), what parameters should the engine use for plausible future trajectories? Population growth rates, technology adoption curves, climate pathways, geopolitical stability assumptions. Output: schema spec for `src/engine/era-loader.ts` to generate future projections.
