...
**Results:** The WWII counterfactual produced statistically significant GDP divergence across all nine nations (Cohen's d > 1.0, Bonferroni-adjusted), with the United States showing the largest mean difference (Δ = +$31.3B, d = 1.13). The climate intervention verified end-to-end sentinel pipeline operation; 14 of 156 metrics reached significance, including occupancy rate (d = 0.70), dialysis sessions (d = 1.11), and CSSD sterilization cycles (d = -2.71). A 30-hospital regional network demonstrated the adapter's distributed operational capability, with occupancy ranging from 40.4% (Sulawesi) to 49.1% (Kalimantan) across five Indonesian regions.
...
- **Universe ID:** Each simulation branch receives a unique ID with parent tracking (e.g., `U-2026-0001-B-2026-0001`), enabling full counterfactual genealogy.
- **Snapshot/restore:** Full world state can be captured at any tick and restored, enabling rewind-point-based counterfactuals.
- **Deterministic resolution order:** All iterable collections (sectors, hospitals, nations) are sorted before processing, guaranteeing iteration order across runs.
...
Event magnitudes are calibrated to historical estimates — for example, a war event applies a −15% GDP modifier to the defender based on WWII-era economic impact assessments [13]. All sector parameters are configurable per era, enabling the same mathematical framework to simulate ancient agrarian economies and modern industrial economies under different parameter regimes.
...
**Temporal aggregation:** One world tick (1 day) equals 1,440 hospital ticks (1 minute each). The adapter runs `fastForward(days)`, stepping the hospital simulator internally and publishing aggregated daily statistics.

**Seed derivation:** Each sentinel derives its RNG seed deterministically from the world seed using a Knuth multiplicative hash:
...
The hash ensures sentinel seeds are uniformly distributed even for adjacent world seeds, preventing correlation artifacts.

**Circuit breaker:** If a sentinel fails to respond for three consecutive ticks, the adapter publishes a `HOSPITAL_DOWN` signal and continues with the last known good state, ensuring macro-level continuity despite micro-level failures.

**Zero modification:** The adapter consumes only the simulator's public API (e.g., `tick()`, `getState()`). Internal state is never accessed directly. The hospital simulator remains independently runnable, testable, and versioned.
...
Each experiment consists of *N* seed pairs. For each seed, the engine forks the baseline universe, applies the intervention, runs both branches for *T* ticks, and captures terminal snapshots. The diff engine compares snapshots, producing per-metric deltas. Statistical significance is assessed using Cohen's d (large effect threshold: |d| > 0.8) with Bonferroni correction for multiple comparisons.
...
|---|---|---|
| War casualties | Simulated casualties (∼1K) vs. historical (∼75M for WWII) — 75,000× gap | Configurable `casualtyMultiplier`; set to ∼2,500 for WWII experiments based on historical records [13] |
| War→GDP sign | Net-positive GDP with war (economic growth outranks 5% GDP hit) | Split attacker/defender: attacker +3% GDP, defender −15% GDP |
...
**Design:** 30 seeds (42–71). Intervention: WWII ends before escalation (Hitler dies 1938). Rewind point: RP-MODERN-001 (1939). Duration: 30 ticks. Total metrics analyzed: 1,421.

**Statistical note:** With 1,421 metrics, an uncorrected α = 0.05 would yield ∼71 expected false positives by chance. We pre-specified nation-level GDP and global climate variables (CO2, temperature, emissions) as primary outcomes and applied Bonferroni correction (adjusted α = 0.05 / 1,421 ≈ 3.5 × 10⁻⁵). The 36 observed significant metrics — fewer than expected by chance — should be interpreted as the correction being conservative rather than the signal being absent; the fact that all nine GDP metrics show large effects (Cohen's d > 1.0) provides cross-validation.
...
**Sector model fidelity:** World sectors use first-order approximations rather than validated macro-economic or climate models. Event magnitudes (e.g., −15% GDP for war defenders) are calibrated to historical estimates [13] but have not undergone formal sensitivity analysis against observational data.
...
**Multiple comparison corrections:** The Bonferroni correction applied to 1,421 metrics (adjusted α ≈ 3.5 × 10⁻⁵) is extremely conservative and likely masks genuine signals in secondary metrics. Future analyses may adopt a false discovery rate (FDR) approach [15] which is more appropriate for exploratory analyses with many correlated metrics.
...
[13] Maddison A. The World Economy: A Millennial Perspective. OECD; 2001.
...
- **Self-harness traces:** 24 traces, 7 failures recorded
